...
import com.coffee.sashie.horadriccube.api.RepresentedClass;
...
public class SkriptYamlRepresenter extends Representer {
	...
	static {		
		if (HoradricCube.getInstance().getServerVersion() <= 12) {
			try {
	...
	public SkriptYamlRepresenter() {
		...
		this.representers.put(HoradricCube.getInstance().getSkriptAdapter().getColorClass(), new RepresentSkriptColor());
	...
	private Node representScalar(Object data) {
		if (data instanceof String && data.toString().contains("&")) {	//fixing a bug with color codes not working sometimes
			if (HoradricCube.getInstance().getServerVersion() >= 13) {
	...
	*/
	...
	private class RepresentSkriptColor implements Represent {
		@Override
		public Node representData(Object data) {
			return representScalar(new Tag("!skriptcolor"), HoradricCube.getInstance().getSkriptAdapter().getColorName(data));
		}
	}
