	...
	public void removeProperty(String path, SkriptNode skriptNode) {
		...
		for (int i = 0; i < parts.length; i++) {
			...
			try {
				...
				HoradricLogger.warn("The node '" + path + "' does not exist, that yaml is empty " + (skriptNode != null ? skriptNode.toString() : ""));
				return;
			}
			// Found our target!
			if (i == parts.length - 1) {
			...
			try {
				...
				HoradricLogger.warn("The node '" + parts[i] + "' likely contains a value therefore the node '" + path + "' does not exist " + (skriptNode != null ? skriptNode.toString() : ""));
				break;
			}
		}
	}
