...
package com.coffee.sashie.horadriccube.utils.yaml;
...
public class YAMLProcessor extends YAMLNode {
	...
	public YAMLProcessor(File file, boolean writeDefaults, YAMLFormat format) {
		...
		Representer representer = HoradricCube.getInstance().getRepresenter();
		representer.setDefaultFlowStyle(format.getStyle());

		//yaml = new Yaml(new SkriptYamlConstructor(), representer, options);
		yaml = new Yaml(HoradricCube.getInstance().getConstructor(), representer, options);
	...
	public void load() throws IOException {
		...
		try {
			...
			try {
				...
				HoradricLogger.error(e, "[Load Yaml] Snakeyaml " + e.getProblem() + " in file '" + file.getAbsolutePath() + "'");	//TODO warn about loss of data since certain nodes didnt load
			}
		} catch (YAMLProcessorException e) {
			root = new LinkedHashMap<String, Object>();
		} finally {
	...
	public String getHeader(SkriptNode skriptNode) {
		if (header != null)
			...
			HoradricLogger.warn("No header found in yaml '" + file.getAbsolutePath() + "' " + skriptNode.toString());
		return null;
	}
	...
	public String getComment(String key, SkriptNode skriptNode) {
		if (comments.containsKey(key))
			...
			HoradricLogger.warn("No comment found at '" + key + "' in yaml " + file.getAbsolutePath()+ " " + skriptNode.toString());
		return null;
	}
	...
	public void setIndent(int indent) {
		try {
			...
			try {
				...
				HoradricLogger.warn(ex.getMessage());
			}
